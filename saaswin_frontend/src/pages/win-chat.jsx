...
const getChatHistory = () => {
    ...
    try {
        ...
        return history ? JSON.parse(history) : []; // { id: timestamp, preview: string }[]
    } catch (error) {
...
const saveChat = (messages) => {
    ...
    try {
        ...
        // 첫 사용자 메시지 + 첫 AI 메시지 일부를 미리보기로 사용
        const userMsg = messages.find((m) => m.role === 'user')?.content;
        const aiMsg = messages.find((m) => m.role === 'assistant')?.content;
        let preview = userMsg ? userMsg.substring(0, 30) : '새 대화';
        if (aiMsg) preview += ` -> ${aiMsg.substring(0, 30)}...`;

        localStorage.setItem(chatId, JSON.stringify(messages));

        const history = getChatHistory();
        history.unshift({ id: timestamp, preview }); // 최신 기록을 맨 위에 추가
        // 기록 개수 제한 (예: 50개)
        if (history.length > 50) {
            const oldestChat = history.pop();
            if (oldestChat) {
                localStorage.removeItem(`chat_${oldestChat.id}`);
            }
        }
        saveChatHistory(history);
        return timestamp; // 저장된 ID (timestamp) 반환
    } catch (error) {
...
const deleteChat = (timestamp) => {
    ...
    try {
        ...
        history = history.filter((item) => item.id !== timestamp);
        saveChatHistory(history);
        return true;
    } catch (error) {
...
function OllamaChatPage() {
    ...
    const scrollContainerRef = useRef(null); // Ref for the scrollable Box

    // --- UI 상태 관리 추가 ---
    const [drawerOpen, setDrawerOpen] = useState(false);
    const [infoDialogOpen, setInfoDialogOpen] = useState(false);
    const [settingsDialogOpen, setSettingsDialogOpen] = useState(false); // 설정 다이얼로그 상태
    const [menuAnchorEl, setMenuAnchorEl] = useState(null);
    const menuOpen = Boolean(menuAnchorEl);
    const [chatHistory, setChatHistory] = useState([]); // 대화 기록 상태
    ...
    const [currentChatId, setCurrentChatId] = useState(null); // 현재 로드된 채팅 ID (timestamp), 새 채팅은 null

    // --- Theme Mode State & Hydration Handling ---
    const [isMounted, setIsMounted] = useState(false);
    const [themeMode, setThemeMode] = useState('light'); // Default light for SSR
    ...
    const sendMessage = async () => {
        if (!input.trim() || loading) return; // Prevent sending empty or during loading

        const isFirstMessage = messages.length === 0; // 자동 저장을 위해 첫 메시지 여부 확인
        ...
        // 첫 메시지이고, 불러온 채팅이 아닐 경우 자동 저장
        if (isFirstMessage && currentChatId === null) {
            const savedId = saveChat(currentMessages); // 현재 메시지(사용자 메시지 포함)로 저장 시도
            if (savedId) {
                setCurrentChatId(savedId); // 자동 저장된 ID를 현재 ID로 설정
                setSnackbarMessage('새 대화가 자동으로 저장되었습니다.');
                setSnackbarOpen(true);
                setChatHistory(getChatHistory()); // 기록 목록 갱신
            } else {
                setSnackbarMessage('자동 저장에 실패했습니다.');
                setSnackbarOpen(true);
            }
        }
        ...
        try {
                    ...
                    try {
                        ...
                        jsonChunks.forEach((jsonChunk) => {
                            ...
                            if (parsed.message && parsed.message.content) {
                                if (firstChunk) {
                                    currentAssistantMessage = parsed.message.content;
                                    setMessages((prev) => {
                                        const updated = [...prev];
            ...
            if (firstChunk && !error) {
                console.warn('Stream ended but no assistant message content was received.');
                setMessages((prev) =>
                    prev.filter((msg, index) => !(index === prev.length - 1 && msg.role === 'assistant'))
                );
            }
        } catch (err) {
            console.error('Chat API Error:', err);
            setError(err.message || 'Failed to get response from AI'); // Set error for MUI Alert
            setMessages((prev) =>
                prev.filter(
    ...
    const handleSettingsOpen = () => {
        ...
        setDrawerOpen(false); // 설정 열면 드로어 닫기
    };
    ...
    const handleClearChat = () => {
        setMessages([]);
        setCurrentChatId(null); // 현재 채팅 ID 초기화
        setInput(''); // 입력창도 비우기
        handleMenuClose();
    ...
    const handleSaveChat = () => {
        // 이미 ID가 있는 대화(로드되었거나 자동저장된)는 덮어쓰지 않고 새 이름으로 저장
        // saveChat 함수가 항상 새 타임스탬프로 저장하므로 별도 로직 불필요
        const savedId = saveChat(messages);
        if (savedId) {
            setSnackbarMessage('대화가 저장되었습니다.');
            setSnackbarOpen(true);
            setChatHistory(getChatHistory()); // Update history list immediately
            // 저장 후 현재 ID를 업데이트 할 수도 있으나, 사용자가 명시적으로 저장한 것이므로
            // currentChatId는 그대로 두어 로드된 상태임을 유지하거나, 업데이트하거나 선택 가능.
            // 여기서는 업데이트하지 않음.
        } else {
            setSnackbarMessage('대화 저장에 실패했습니다.');
            setSnackbarOpen(true);
        }
        handleMenuClose();
    };

    const handleLoadChat = (timestamp) => {
        const loadedMessages = loadChat(timestamp);
        setMessages(loadedMessages);
        setCurrentChatId(timestamp); // 로드된 채팅 ID 설정
        setSnackbarMessage('대화를 불러왔습니다.');
        setSnackbarOpen(true);
        setDrawerOpen(false); // Close drawer after loading
    };

    const handleDeleteChat = (timestamp) => {
        if (window.confirm('이 대화 기록을 삭제하시겠습니까? 되돌릴 수 없습니다.')) {
            const success = deleteChat(timestamp);
            if (success) {
                setChatHistory(getChatHistory()); // Update history list
                setSnackbarMessage('대화 기록이 삭제되었습니다.');
                setSnackbarOpen(true);
                // 만약 현재 로드된 채팅이 삭제된 것이라면 초기화
                if (currentChatId === timestamp) {
                    handleNewChat(false); // 확인 메시지 없이 새 채팅 시작
                }
            } else {
                setSnackbarMessage('기록 삭제에 실패했습니다.');
                setSnackbarOpen(true);
            }
        }
    };
    ...
    const handleNewChat = (showSnackbar = true) => {
        ...
        // Optionally close drawer or menu if open
        setDrawerOpen(false);
        setMenuAnchorEl(null);
    };
        ...
        <ThemeProvider theme={theme}>
            ...
            >
                ...
                <List>
                    ...
                    <Divider sx={{ my: 1 }}>대화 기록</Divider>
                    {chatHistory.length === 0 ? (
                        <ListItem>
                            <ListItemText
                                secondary='저장된 대화 기록이 없습니다.'
                        ...
                        chatHistory.map((item) => (
                            <ListItem
                                ...
                                secondaryAction={
                                    <Box sx={{ display: 'flex', gap: 0.5 }}>
                                        ...
                                        >
                                            <LoadIcon fontSize='inherit' color='success' />
                                        </IconButton>
                                        <IconButton
                                            edge='end'
                                            aria-label='delete'
                            ...
                            >
                                <ListItemButton onClick={() => handleLoadChat(item.id)}>
                                    <ListItemText
                    ...
                    <ListItem disablePadding>
                        <ListItemButton onClick={handleClearChat}>
                            ...
                            <ListItemText primary='대화 내용 지우기' primaryTypographyProps={{ color: 'error' }} />
                        </ListItemButton>
                    </ListItem>
                </List>
                <Box sx={{ flexGrow: 1 }} />
            ...
            >
                ...
                >
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        >
                            <MenuItem onClick={handleSaveChat} sx={{ gap: 1 }}>
                                <SaveIcon fontSize='small' color='primary' />
                                <Typography variant='body2'>대화 저장</Typography>
                            </MenuItem>
                ...
                <Dialog
                    ...
                    PaperProps={{
                        elevation: 5,
                        sx: { borderRadius: 3, maxWidth: '500px' },
                    }}
                >
                    ...
                    <DialogContent>
                        <DialogContentText id='info-dialog-description' sx={{ mb: 2 }}>
                            WIN_AI 1.0는 Ollama를 기반으로 하는 로컬 실행 대화형 AI 어시스턴트입니다.
                        </DialogContentText>
                        <Typography variant='subtitle2' color='primary' gutterBottom sx={{ fontWeight: 600, mt: 2 }}>
                            사용 방법
                        </Typography>
                        <Typography variant='body2' paragraph>
                            하단의 입력창에 질문이나 대화를 입력하고 전송 버튼을 누르거나 Enter 키를 누르세요. AI가
                            생성한 답변이 실시간으로 표시됩니다.
                        </Typography>
                        ...
                        <Typography variant='body2' component='div'>
                            <ul style={{ paddingLeft: '1.5rem', marginTop: '0.5rem', marginBottom: '0.5rem' }}>
                                <li>자연어 대화 기능</li>
                                <li>다크 모드 지원</li>
                                <li>로컬 실행 (데이터가 외부로 전송되지 않음)</li>
                                <li>대화 내용 저장 및 내보내기</li>
                            </ul>
                        </Typography>
                        ...
                        <Typography variant='body2' paragraph>
                            AI가 생성한 정보는 항상 정확하지 않을 수 있으니 중요한 정보는 반드시 추가로 확인하세요.
                        </Typography>
                    </DialogContent>
                    <DialogActions sx={{ px: 3, pb: 3 }}>
                        <Button variant='outlined' onClick={handleInfoClose} color='primary'>
                            닫기
                        </Button>
                    </DialogActions>
                </Dialog>
                ...
                >
                    ...
                    >
                        ...
                        {/* Message Bubbles with Enhanced Styling and Animation */}
                        {messages.map((message, index) => (
                            ...
                            >
                                ...
                                >
                                    ...
                                    {/* Message Bubble with Background Gradient and Animations */}
                                    <Paper
                                        elevation={2}
                                        sx={{
                ...
                >
                    ...
                    >
                        {/* Enhanced TextField with animations */}
                        <TextField
                            fullWidth
                            variant='outlined'
                            size='small'
