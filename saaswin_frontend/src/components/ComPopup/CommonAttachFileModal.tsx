...
const CommonAttachFileModal = ({ params, setParams, handleUpload, allowMultiple }: AttachFileModalProps) => {
    // 로그인한 사용자번호
    const userNo = useAuthStore((state) => state.userNo);
    const [attachFileList, setAttachFileList] = useState([]);
    const { isEditable, procType, value } = params;
    // 로딩
        ...
        <SwModal open={params.open} onClose={onClose} title='첨부 파일' size='md' className={styles.attachFile}>
            <section className={`${styles.attachFile} ${styles.section}`}>
                <div className={styles.fileTitle}>
                    ...
                    {!allowMultiple && (
                        <Typography title='첨부 파일' type='section' className={styles.sectionTit}>
                            첨부파일은 한 개만 등록할 수 있습니다. 새 파일을 추가하려면 기존 파일을 삭제해 주세요.
                        </Typography>
                    )}
                    {/* 수정 가능할 때만 추가 버튼 표시 */}
                    {!isEditable && (
                        <Button
                            ...
                            type='default'
                            size='sm'
                            disabled={!allowMultiple && attachFileList.length >= 1}
                ...
                <div className='dragNdropFile'>
                    {attachFileList.length === 0 ? (
                        <FileUploader
                            ...
                            multiple={allowMultiple}
                            disabled={isEditable || (!allowMultiple && attachFileList.length >= 1)} // 수정 불가능하면 비활성화
                        >
                            {loading ? (
...
export default CommonAttachFileModal;
