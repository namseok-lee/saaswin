...
const AttachFile: React.FC<AttachFileProps> = ({ params, setParams, procType, formMode }) => {
    // 로그인한 사용자번호
    ...
    const handleFileChange = (files: File | FileList) => {
        ...
        // 유효한 파일만 추가 (중복 제거)
        const newFiles = validFiles.filter((newFile) => !params.some((existFile) => existFile.name === newFile.name));
        ...
        try {
            ...
            fetcherPostFile(newFiles[0], params)
                .then((fileId) => {
                    // 썸네일 생성 후 파일 ID 저장
                    fetcherGetFileInfo(fileId)
                        .then((response) => {
                            // 파일 추가
                            setParams((prevFiles) => [...prevFiles, response]);
                        })
                        .catch((error) => {
                            console.error(error);
                        });
    ...
    const handleFileRemove = (file) => {
        //setParams((prevFiles) => prevFiles.filter((_, index) => index !== indexToRemove));
        setParams((prevFiles) => prevFiles.filter((prevFiles) => prevFiles.file_id !== file.file_id));
    };
            ...
            <section className={`${styles.attachFile} ${styles.section}`}>
                ...
                <div className='dragNdropFile'>
                    ...
                    <div className='fileSize'>
                        ( {params.length > 0 ? `${params.reduce((acc, file) => acc + file.orgnfl_sz, 0)} KB` : '0 MB'}{' '}
                        /20MB)
                    </div>
