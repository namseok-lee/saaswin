...
interface UserData {
    ...
    crr_info?: any;
    eldoc_file_info?: any;
    prsnl_info_cnfrm_ymd?: string;
    prsnl_info_cnfrm_user_nm?: string;
    [key: string]: unknown;
}
...
export const cardData = [
    {
        ...
        content: (userData: UserData | null) => {
            if (!userData || !userData.bsc_info) {
                return <Empty>데이터가 없습니다.</Empty>;
            }
            const bsc = userData.bsc_info as Record<string, string>;
            return (
                <>
                    {bsc.korn_flnm && (
                        <div className='primary'>
                            <div className='emphasis'>{bsc.korn_flnm}</div>
                            {bsc.eng_flnm && <div className='emphasis'>{bsc.eng_flnm}</div>}
                        </div>
                    )}
                    {bsc.brdt && (
                        <div className='infoRow'>
                            <div className='rowTitle'>생년월일</div>
                            <div className='rowText'>{dayjs(bsc.brdt).format('YYYY.MM.DD')}</div>
                        </div>
                    )}
                    {bsc.ntn_cd && (
                        <div className='infoRow'>
                            <div className='rowTitle'>국적</div>
                            <div className='rowText'>{bsc.ntn_cd}</div>
                        </div>
                    )}
                    {bsc.jncmp_ymd && (
                        <div className='infoRow'>
                            <div className='rowTitle'>입사일</div>
                            <div className='rowText'>{dayjs(bsc.jncmp_ymd).format('YYYY.MM.DD')}</div>
                        </div>
                    )}
                    {bsc.reg_no && (
                        <div className='infoRow'>
                            <div className='rowTitle'>주민번호</div>
                            <div className='rowText'>{bsc.reg_no}</div>
                        </div>
                    )}
                </>
            );
                ...
                <>
                    ...
                    {telno_info?.tel_no && (
                        <div className='infoRow'>
                            ...
                            <div className='rowText'>{telno_info?.tel_no || '-'}</div>
                        </div>
                    )}
                    {isExtNo && (
                        <div className='infoRow'>
                            <div className='rowTitle'>내선번호</div>
                            <div className='rowText'>{isExtNo?.tel_no || '-'}</div>
                        </div>
                    )}
                </>
            );
    ...
    },
    {
        ...
        content: (userData: UserData | null) => {
            ...
            const lastEdu = userData.acbg_info[userData.acbg_info.length - 1];
            return (
                <>
                    {lastEdu.schl_nm && (
                        <div className='primary'>
                            <div className='emphasis'>학사</div>
                            <div className='emphasis'>{lastEdu?.schl_nm || '-'}</div>
                            <div className='emphasis'>{lastEdu?.grdtn_cd || '-'}</div>
                        </div>
                    )}
                    {lastEdu.mtcltn_ym && (
                        <div className='infoRow'>
                            ...
                            <div className='rowText'>
                                {dayjs(lastEdu?.mtcltn_ym).format('YYYY.MM')} ~{' '}
                                {dayjs(lastEdu?.grdtn_ym).format('YYYY.MM')}
                            </div>
                        </div>
                    )}
                    {lastEdu.scsbjt && (
                        <div className='infoRow'>
                            ...
                            <div className='rowText'>{lastEdu?.scsbjt || '-'}</div>
                        </div>
                    )}
                    {lastEdu.mjr && (
                        <div className='infoRow'>
                            <div className='rowTitle'>전공</div>
                            <div className='rowText'>{lastEdu?.mjr || '-'}</div>
                        </div>
                    )}
                    {lastEdu.dbl_mjr && (
                        <div className='infoRow'>
                            <div className='rowTitle'>복수전공</div>
                            <div className='rowText'>{lastEdu?.dbl_mjr || '-'}</div>
                        </div>
                    )}
                    {lastEdu.mnr_mjr && (
                        <div className='infoRow'>
                            <div className='rowTitle'>부전공</div>
                            <div className='rowText'>{lastEdu?.mnr_mjr || '-'}</div>
                        </div>
                    )}
                </>
            );
    ...
    },
    {
        ...
        content: (userData: UserData | null) => {
            ...
            const recentLgsdy = userData.lgsdy_info
                .slice()
                .sort((a, b) => b.lgsdy_acqs_ymd.localeCompare(a.lgsdy_acqs_ymd))
                .slice(0, 2);
            return (
                <>
                    {recentLgsdy.map((item, index) => (
                        <Fragment key={`lgsdy-${index}`}>
                            ...
                            {item.lgsdy_acqs_ymd && item.lgsdy_exp_ymd && (
                                <div className='infoRow'>
    ...
    },
    {
        ...
        content: (userData: UserData | null) => {
            ...
            const recentEdu = userData.edu_info
                .slice()
                .sort((a, b) => b.edu_end_ymd.localeCompare(a.edu_end_ymd))
                .slice(0, 2);
            return (
                <>
                        ...
                        <Fragment key={`edu-${index}`}>
                            <div className='primary'>
                                <div className='emphasis'>
                                    ...
                                    <div className='text'>
                                        {dayjs(item.edu_start_ymd).format('YYYY.MM.DD')} ~{' '}
                                        {dayjs(item.edu_end_ymd).format('YYYY.MM.DD')}
                                    </div>
                                </div>
                            </div>
                            <div className='infoRow'>
    ...
    },
    {
        ...
        content: (userData: UserData | null) => {
            ...
            // 빈 객체 체크 함수
            const isEmpty = (obj: any) => obj == null || (typeof obj === 'object' && Object.keys(obj).length === 0);
    ...
    },
    {
        ...
        content: (userData: UserData | null) => {
            ...
            const recentRwdptr = rwdptr_info
                .slice()
                .sort((a, b) => b.rwdptr_reg_ymd.localeCompare(a.rwdptr_reg_ymd))
                .slice(0, 2);
            return (
                <>
                                ...
                                {item.rwdptr_reg_ymd && (
                                    <>
                                        <div className='infoRow'>
                                            ...
                                            <div className='rowText'>
                                                {dayjs(item.rwdptr_reg_ymd).format('YYYY.MM.DD')}
                                                {item.rwdptr_rmv_ymd
                                                    ? ` ~ ${dayjs(item.rwdptr_rmv_ymd).format('YYYY.MM.DD')}`
                                                    : ''}
                                            </div>
                                        </div>
                                    </>
