...
import { IcoAddPerson } from '@/assets/Icon';
...
export default function Ssw01_x({ TabmasterUI }: { TabmasterUI: MasterUIData[] }) {
    ...
    const handleMoveToInvtn = () => {
        setEmptyResponseModal(false);
        router.push('/settings/systemSetting/inviteManagement/SF--1');
    };
        ...
        if (masterRetrieve) {
            ...
            if (item2[0].sqlId === '0' || item2[0].sqlId === 0) {
                ...
                fetcherPostGridData(item)
                    .then((response) => {
                        if (sqlKey === 'hpr_invtn_trpr_get') {
                            console.log('hpr_invtn_trpr_get', response);

                            // 빈 배열 체크 및 모달 열기
                            if (Array.isArray(response) && response.length === 0) {
                                console.log('응답이 빈 배열입니다.');
    ...
    return (
        <div>
            ...
            {emptyResponseModal && (
                <SwModal open={emptyResponseModal} onClose={() => setEmptyResponseModal(false)} size='md'>
                    <div className='cont'>
                        <div className='headIco'>
                            <IcoAddPerson fill='var(--primary)' />
                        </div>
                        <div className='emphasis'>구성원을 초대하고 다 함께 인사잇을 시작해 보세요!</div>
                    </div>
                    <div className='msg'>
                        아직 우리 회사만의 초대 양식을 만들지 못했다면
                        <br />
                        &apos;환경설정 초대 관리&lsquo;에서 맞춤 양식을 먼저 설정해주세요!
                    </div>
                    <div className='actions'>
                        <Button type='primary' size='lg' onClick={() => handleMoveToInvtn()}>
                            초대 관리 바로가기
                        </Button>
                        <Button type='default' onClick={() => setEmptyResponseModal(false)} size='lg'>
                            닫기
                        </Button>
                    </div>
                </SwModal>
