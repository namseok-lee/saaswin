...
export default function Ssw01t_tg({ TabmasterUI }: { TabmasterUI: MasterUIData[] }) {
    ...
    const tableButtonData = TabmasterUI?.table_btn_info;
    const tableData = TabmasterUI?.table_info;
    ...
    useEffect(() => {
        ...
        if (initRetrieve) {
            if (dataParam?.master?.[0]?.params[0]) {
                const params = dataParam?.master[0].params[0];
                const BtnArray = masterUI?.table_btn_info;
