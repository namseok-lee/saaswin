...
export default function HrInfoProfile({
        ...
        memberList.forEach((item) => {
            ...
            if (user_no) {
                ...
                usersByOrg.get(ognz_no).push({
                    ...
                    ognz_nm,
                    jbps_cd: apnt_info?.jbps_cd,
                    jbttl_cd: apnt_info?.jbttl_cd,
                    jbgd_cd: apnt_info?.jbgd_cd,
                    apnt_grdslry_cd: apnt_info?.apnt_grdslry_cd,
                    ttl_cd: apnt_info?.ttl_cd,
                    // 추가 info 필드들도 포함
                    actno_info,
                    comut_info,
                    bztrp_info,
    ...
    const filteredMembers = useMemo(() => {
        ...
        const allMembers = memberList
            ...
            .map((item) => ({
                ...
                jbps_cd: item.apnt_info?.jbps_cd,
                jbttl_cd: item.apnt_info?.jbttl_cd,
                jbgd_cd: item.apnt_info?.jbgd_cd,
                grdslry_cd: item.apnt_info?.grdslry_cd,
                ttl_cd: item.apnt_info?.ttl_cd,
    ...
    const isSearchMatched = useCallback(
        (member: MemberData) => {
            ...
            return [member.flnm, member.ognz_nm, member.jbps_cd, member.jbttl_cd]
                .filter(Boolean)
                .some((field) => field!.toLowerCase().includes(keyword));
        },
                        ...
                        <ul className={styles.infoList}>
                            ...
                            {apnt_info?.jbps_cd && (
                                <li className={styles.item}>
                                    ...
                                    <div className={styles.text}>{apnt_info.jbps_cd}</div>
                                </li>
                            )}
                            {apnt_info?.jbttl_cd && (
                                <li className={styles.item}>
                                    <span className={styles.title}>직책</span>
                                    <div className={styles.text}>{apnt_info.jbttl_cd}</div>
                                </li>
                            )}
                            {apnt_info?.jbgd_cd && apnt_info?.jbgd_cd !== '직급없음' && (
                                <li className={styles.item}>
                                    <span className={styles.title}>직급</span>
                                    <div className={styles.text}>{apnt_info.jbgd_cd}</div>
                                </li>
                            )}
                            ...
                            {apnt_info?.ttl_cd && apnt_info?.ttl_cd !== '호칭없음' && (
                                <li className={styles.item}>
                                    <span className={styles.title}>호칭</span>
                                    <div className={styles.text}>{apnt_info.ttl_cd}</div>
                                </li>
                            )}
                    ...
                    {selectedMember ? (
                        <div className={styles.memberGroupInfo}>
                            ...
                            <ul className={styles.infoList}>
                                ...
                                {selectedMember?.jbps_cd && (
                                    <li className={styles.item}>
                                        <span className={styles.title}>직위 </span>
                                        <div className={styles.text}>{selectedMember.jbps_cd}</div>
                                    </li>
                                )}
                                {selectedMember?.jbttl_cd && (
                                    <li className={styles.item}>
                                        <span className={styles.title}>직책</span>
                                        <div className={styles.text}>{selectedMember.jbttl_cd}</div>
                                    </li>
                                )}
                                {selectedMember?.jbgd_cd && selectedMember?.jbgd_cd !== '직급없음' && (
                                    <li className={styles.item}>
                                        <span className={styles.title}>직급</span>
                                        <div className={styles.text}>{selectedMember.jbgd_cd}</div>
                                    </li>
                                )}
                                ...
                                {selectedMember?.ttl_cd && selectedMember?.ttl_cd !== '호칭없음' && (
                                    <li className={styles.item}>
                                        <span className={styles.title}>호칭</span>
                                        <div className={styles.text}>{selectedMember.ttl_cd}</div>
                                    </li>
                                )}
